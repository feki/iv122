...
task :build, [:dir_name] => [:clean, :copy_files] do |t, args|
  ...
  File.open("#{dir_name}/11.html", 'w') { |f| f.write Slim::Template.new('site/11.slim').render }
  File.open("#{dir_name}/12.html", 'w') { |f| f.write Slim::Template.new('site/12.slim').render }
end